...
RSpec.describe WordInfo::Adjective::Format::Text do
  ...
  before do
    allow(WordInfo::Adjective::Format::Heading).to receive(:call).with(item:).and_return(double(heading: heading))
    allow(WordInfo::Adjective::Format::Body).to receive(:call).with(item:).and_return(double(body: body))
  end
